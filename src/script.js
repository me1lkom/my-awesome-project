...
function showError(input, massage) { // функция покаха ошибки
    const formControl = input.parentElement; // получаем родительскую форму
    const errorElement = formControl.querrySelector('.error') || document.createElement('div'); //ищем существующий блок ошибки, если нет создаём

    errorElement.class = 'error'; // css стиль для блока ошибки
    errorElement.textContent = massage; // задаём сообщение об ошибки из параметра функции
...
function removeError(input) { // функция скрытия ошибки
    const formControl = input.parentElement; // получаем родительскую форму
    const errorElement = formControl.querrySelector('.error') // ищем существующий блок ошщибки
